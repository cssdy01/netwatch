...
app.get('/healthz', (req, res) => res.json({ status: 'ok', service: 'netwatch-backend' }));
...
async function startup() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[NetWatch] Backend running on port ${PORT}`);
    // Log the startup event using the compliant ADMIN category
    log('INFO', 'ADMIN', 'system', null,
      `NetWatch started on port ${PORT} — monitor host: ${process.env.MONITOR_HOST || 'not-set'}`,
